...
const cr = await call(a, 'create', { name: 'Тест', chain: ['doors', 'unique'], maxPlayers: 6 });
...
for (const [s, id] of [[a, 'A'], [b, 'B']]) {
  s.on('game', (st) => {
    ...
    if (st.phase !== 'act' || !st.alive.includes(me.id) || st.done.includes(me.id)) return;
...
const t0 = Date.now();
while (!final && Date.now() - t0 < 240000) await new Promise((r) => setTimeout(r, 500));
